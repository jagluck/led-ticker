...
void tickStaticClock()
{
  ...
  if (!getLocalTime(&t, 0))
    return;
  if (staticClockLastMin == t.tm_min)
    return;
...
void applyPendingDisplayCfg()
{
  ...
  *sep = '\0';
  ...
  if (scroll < SCROLL_SPEED_MIN_MS)
    scroll = SCROLL_SPEED_MIN_MS;
  if (scroll > SCROLL_SPEED_MAX_MS)
    scroll = SCROLL_SPEED_MAX_MS;
  ...
  display.setIntensity(displayBrightness);
  if (currentMode != MODE_SETUP)
    display.setSpeed(scrollSpeedMs);
...
static void applyPendingUpdates()
{
  ...
  if (powerUpdatePending)
    applyPendingPower();
  if (displayCfgUpdatePending)
    applyPendingDisplayCfg();
  if (tzUpdatePending)
