...
- **Pick the right font face.** Everything steady — text, signs, clock, timer — uses the Press Start 2P static face (`useStaticFont`); scrolling uses the thin system font (`useScrollFont`). Press Start 2P's digits are hand-condensed to 6px (in `tools/gen_pressstart_font.py`) so a five-glyph time (clock `HH:MM`, timer `MM:SS`) fits the 32-col panel — the original 7px digits would overflow. `bold_font.h` stays compiled as a future BLE-selectable second face. Faces are generated by `tools/gen_pressstart_font.py` / `tools/gen_bold_font.py`.
